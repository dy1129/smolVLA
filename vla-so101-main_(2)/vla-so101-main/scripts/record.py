...
def reset_to_home(robot: SO101Follower, cfg: dict) -> None:
    ...
    bus.sync_write("Goal_Position", action_dict, num_retry=3)
    precise_sleep(cfg["safe_joint_wait_s"])
...
def main():
    ...
    # Build default processors required by record_loop
    ...
    with VideoEncodingManager(dataset):
        ...
        while recorded_episodes < num_episodes and not events["stop_recording"]:
            ...
            try:
                ...
                logging.error(f"Connection lost during recording: {e}")
                log_say("Connection lost. Stopping recording.", play_sounds=True)
                # Discard the incomplete episode
                dataset.clear_episode_buffer()
                break

            # Reset exit_early flag for next episode
            ...
            # Only save if there are frames in the buffer
            if dataset.episode_buffer is not None and dataset.episode_buffer.get("size", 0) > 0:
                dataset.save_episode()
                recorded_episodes += 1
                logging.info(f"Saved episode {recorded_episodes}/{num_episodes}")
            else:
                logging.warning("Empty episode buffer, skipping save")
                dataset.clear_episode_buffer()
    ...
    try:
        ...
        logging.warning("Robot disconnect failed (already disconnected)")
    try:
        teleop.disconnect()
    except Exception:
        logging.warning("Teleop disconnect failed (already disconnected)")
    ...
    if recorded_episodes > 0:
        logging.info(f"Pushing {recorded_episodes} episodes to HuggingFace Hub...")
        dataset.push_to_hub()
        logging.info(f"Dataset pushed to https://huggingface.co/datasets/{repo_id}")
    else:
        logging.warning("No episodes recorded, skipping push to hub.")
